...
class SelfRepresentation(BaseEstimator, ClusterMixin):
    ...
    def _representation_to_affinity(self):
        ...
        if self.affinity == 'symmetrize':
            self.affinity_matrix_ = 0.5 * (np.absolute(normalized_representation_matrix_) + np.absolute(
                normalized_representation_matrix_.T))
...
class ElasticNetSubspaceClustering(SelfRepresentation):
    ...
    def __init__(self, n_clusters=8, affinity='symmetrize', random_state=None, n_init=20, n_jobs=1, gamma=50.0,
                 gamma_nz=True, tau=1.0,
...
class SparseSubspaceClusteringOMP(SelfRepresentation):
    ...
    def __init__(self, n_clusters=8, affinity='symmetrize', random_state=None, n_init=10, n_jobs=1, n_nonzero=10,
                 thr=1.0e-6):
        self.n_nonzero = n_nonzero
        self.thr = thr
        SelfRepresentation.__init__(self, n_clusters, affinity, random_state, n_init, n_jobs)
    ...
    def _self_representation(self, X):
        self.representation_matrix_ = sparse_subspace_clustering_orthogonal_matching_pursuit(X, self.n_nonzero,
                                                                                             self.thr)
...
class LeastSquaresSubspaceClustering(SelfRepresentation):
    ...
    def __init__(self, n_clusters=8, affinity='symmetrize', random_state=None, n_init=None, n_jobs=1, gamma=10.0,
                 exclude_self=False):
        self.gamma = gamma
...
def exact_alm_lrr_l21v2(D, A, lamb, tol=1e-7, maxIter=1000, display=False):
    ...
    Y = np.sign(D)
    norm_two = np.linalg.norm(Y, 2)
    norm_inf = np.linalg.norm(Y.flatten(), np.inf) / lamb
    dual_norm = max(norm_two, norm_inf)
    Y /= dual_norm
...
def low_rank_representation(X, A, lamb, reg=0, alm_type=0, display=False):
    Q = orth(A.T)
    B = A.dot(Q)
...
class LowRankRepresentation(SelfRepresentation):
    def __init__(self, n_clusters=8):
        SelfRepresentation.__init__(self, n_clusters)

    def _self_representation(self, X):
        data = X.T
        A = data
        self.representation_matrix_ = low_rank_representation(data, A, lamb=0.1)[0]


def block_diagonal_representation(X, k, lam, gamma):
    n = len(X[0])
    m = len(X)
    tol = 1e-3
    maxIter = 1000
    ...
    while iter < maxIter:
        ...
        Bk = B
        print(Z)
        # print("Z, W" , len(Z), len(W))
        # print("Z, W", len(Z[0]), len(W[0]))
        B = (1 - gammaoverlambda) * Z
        # B = max(0, (B + B)/2)
        B = B - np.diag(B)
        L = np.diag(B * one) - B

        # update W
        data = L.tolist()  ##作为矩阵的data，先将其转化为列表
        ###添加0，使二维温度矩阵变为方阵
        for item1 in data:
            if len(item1) != len(data):
                if len(item1) < len(data):
                    for _ in range(len(data) - len(item1)):
                        item1.append(0)  ##矩阵行大于列，对列进行补零
                else:
                    data.append([0 for _ in range(len(item1))] for _ in range(len(item1) - len(data)))  ##矩阵列大于行，对行进行补零
                    break
        data1 = np.array(data)  ###作为方阵的data1
        D, V = np.linalg.eig(data1)
        D = np.diag(D)
        # [b, ind] = sort(D)
        W = V[:, random.randint(1,k)]*V[:, random.randint(1,k)].transpose()

        diffZ = np.max(np.max(abs(Z - Zk)))
        diffB = np.max(np.max(abs(B - Bk)))
...
class BlockDiagonalRepresentation(SelfRepresentation):
    def __init__(self, n_clusters=8):
        SelfRepresentation.__init__(self, n_clusters)
    ...
    def _self_representation(self, X):
        lamb = 50
        gamma = 1
        self.representation_matrix_ = block_diagonal_representation(X, self.n_clusters, lamb, gamma)[0]